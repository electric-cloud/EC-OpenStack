...
// OpenStackConfigListLoader.java --
//
// OpenStackConfigListLoader.java is part of ElectricCommander.
//
...
public class OpenStackConfigListLoader
    extends Loader
{

    //~ Instance fields --------------------------------------------------------

    private final OpenStackConfigList   m_configList;
    private final CgiRequestProxy m_cgiRequestProxy;
    private String                m_editorName;

    //~ Constructors -----------------------------------------------------------

    public OpenStackConfigListLoader(
            OpenStackConfigList   configList,
            Component       component,
            ...
            ChainedCallback callback)
    {
        ...
        m_cgiRequestProxy = new CgiRequestProxy(getPluginName(), "openstack.cgi");
    }
    ...
    private void loadConfigs(Map<String, String> cgiParams)
    {
        ...
        try {
                    ...
                    new RequestCallback() {
                                ...
                                Throwable exception)
                        {
                            ((HasErrorPanel) m_component).addErrorMessage(
                                "Error loading OpenStack configuration list: ",
                                exception);
                        }
            ...
            if (m_component instanceof HasErrorPanel) {
                ((HasErrorPanel) m_component).addErrorMessage(
                    "Error loading OpenStack configuration list: ", e);
            }
            else {
                m_component.getLog()
                           .error(e);
    ...
    private void loadEditors()
    {
        ...
        request.setPropertyName("/plugins/EC-OpenStack/project/ui_forms/"
                + m_editorName);
        request.setExpand(false);
        request.setCallback(new EditorLoaderCallback("openstackcfg"));
        ...
        implements PropertyCallback
    {
        ...
        @Override public void handleResponse(Property response)
        {
            ...
            String errorMsg = "Editor '" + m_editorName
                    + "' not found for OpenStack plugin '" + m_configPlugin + "'";
